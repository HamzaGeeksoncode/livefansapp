...
class PostController extends Controller
{
    ...
    public function getPostList(Request $request)
    {
        ...
        if ($type == "following") {
            ...
            $post_list = $query->orderBy('post_id', 'DESC')->offset($start)->limit($limit)->get();
        } else if ($type == "trending") {
            $post_list = Post::where('is_trending', 1)->orderBy('post_id', 'DESC')->offset($start)->limit($limit)->get();
        } else {
            $post_list = Post::inRandomOrder()->offset($start)->limit($limit)->get();
        }
    ...
    public function getSearchSoundList(Request $request)
    {
        ...
        if ($keyword) {
            $soundList = Sound::where('is_deleted', 0)->where('added_by', 'admin')->where('sound_title', 'LIKE', "%{$keyword}%")->orderBy('sound_id', 'DESC')->get();
        } else {
    ...
    public function getFavouriteSoundList(Request $request)
    {
        ...
        $soundData =  Sound::whereIn('sound_id', $sound_ids)->get();
